...
var canMove = true; //возможность двигаться
var yourColor = true; //цвет игрока
var chatEnable = false; //возможность отправлять сообщения
//pawn - пешка
//rook - ладья
//khight - конь
//bitshop - слон
...
var proverka = function(x,y, itemColor, color){ //проверяет фигура ли противника находится под возможным ходом
	...
	{
		return true;//если противника true
	} else{
		return false;
	}
}
...
function changePosition(x, y, xNew, yNew) {              //смена позиции
	if (canWeChangePosition(xNew, yNew)) {
		...
		//socket.emit('step',x ,y, xNew,yNew) //отправляем на сервер координаты, где находится, куда идет
		
		if (globColor != curColor && possibleMat)
		{
		//шах уже был поставлен, проверяем на возможный мат
			Shah(globX, globY);                  
			possibleMat = false;
		}else{
			Shah(xNew,yNew);
...
$(document).ready(function(){
	...
	socket.on('start', function(color){
		...
		if (chatEnable) {
			$(".chatInput").prop('disabled', '');//чат становиться доступным
		}

		if (color == 'white')
		{
			canMove = true;  //нужно, чтобы не дать игроку ходить, когда не его ход
        ...
        var userColorText = yourColor ? 'белыми' : 'чёрными'; //у белых yourColor - true
        $(".user-color").text('Ты играешь ' + userColorText); //у черных yourColor - false
	...
	$('td').on('click', function() {
		if (!isConnectionEstablished) return; //пока не произошло подключение, ходить не получиться
		if (blackOrWhite != yourColor) return;//пока не твой цвет, тоже самое
		...
		if(blKing == false)
		{
			socket.emit('finish'); //отправляем на сервер конец игры
